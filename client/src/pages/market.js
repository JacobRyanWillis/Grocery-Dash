...
const Market = () => {
  ...
  const renderSection = (category) => {
    ...
    return (
      <Link to="/shop" state={{ owner: randomOwner?.myProducts }}>
  ...
  const renderCarouselItems = (features) => {
    const slicedFeatures = features.slice(0, 5); // Slice the features array to get the first 5 items
    return slicedFeatures.map((feature) => (
      <div className="m-2 flex flex-col justify-center items-center" key={feature._id}>
        <img
          className="h-24 w-24 rounded-full"
          alt={feature.productName}
          src={feature.image}
        />
        <p className="md:text-lg">{feature.productName}</p>
      </div>
    ));
  };
  ...
  return (
    <div className="font-gilroy">
      ...
      <div className="bg-light-tan">
        <Slider {...settings}>{renderCarouselItems(filteredFeature)}</Slider>
      </div>
