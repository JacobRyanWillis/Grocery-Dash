...
const Shop = () => {
  ...
  const handleCategories = (selectedOption) => {
    ...
    setPageTitle(selectedOption.value);
    setOwnerStory("")
  ...
  const vendors = () => {
    ...
    const filteredVendors = ownersData?.map((owner) => {
      return { value: `${owner.ownerName}`, label: `${owner.ownerName}` };
    });
  ...
  const handleVendors = (selectedOption) => {
    ...
    /* setOwnerStory(vendor[0].ownerStory) */
    setOwnerStory("")
